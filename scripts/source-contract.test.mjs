...
function requireSessionFsProviderAliases(source) {
  assert.deepEqual(
    sourceContract.plainOmitAlias(source, "SessionFsFileInfo"),
    {
      base: "SessionFsStatResult",
  ...
  assert.deepEqual(
    sourceContract.plainOmitAlias(source, "SessionFsSqliteQueryResult"),
    {
      base: "GeneratedSqliteQueryResult",
      excluded: ["error"],
    },
  );
  requireExactTypeImportBinding(
...
test("SessionFS provider aliases pin bases exclusions and generated import provenance", () => {
  const valid = `
import type {
    SessionFsHandler,
    /* imported */ SessionFsSqliteQueryResult
...
export type SessionFsFileInfo = Omit<SessionFsStatResult, "error">;
export type SessionFsSqliteQueryResult =
  Omit<GeneratedSqliteQueryResult, "error">;
`;

  assert.equal(requireSessionFsProviderAliases(valid), undefined);

  for (const source of [
    valid.replace(
      "Omit<SessionFsStatResult",
      "Omit<GeneratedSqliteQueryResult",
    ),
    valid.replace('"error">;', '"message">;'),
    valid.replace(
      "/* imported */ SessionFsSqliteQueryResult",
      "/* imported */ OtherResult",
    ),
    valid.replace(
      'GeneratedSqliteQueryResult,',
      'OtherGeneratedSqliteQueryResult,',
    ),
    valid.replace(
      "from './generated/rpc.js'",
      "from './other.js'",
    ...
    assert.throws(
      () => requireSessionFsProviderAliases(source),
      /SessionFS|Expected values to be strictly deep-equal/,
    );
  }
});
...
test("SessionFS registration interfaces reject every property drift category", () => {
  const contracts = {
...
`;
  for (const [name, expected] of Object.entries(contracts)) {
    requireExactPropertySignatures(
      interfacePropertySignatures(valid, name),
      expected,
      name,
    );
  }
  ...
  const drifts = [
    ...
    [
      "removed",
      valid.replace("  sessionStatePath: string;\n", ""),
      "SessionFsSetProviderRequest",
    ],
    [
      "renamed",
      valid.replace("  success: boolean;", "  accepted: boolean;"),
      "SessionFsSetProviderResult",
    ],
    [
      "type",
      valid.replace("  initialCwd: string;", "  initialCwd: URL;"),
      "SessionFsSetProviderRequest",
    ],
    [
      "optionality",
  ...
  for (const [kind, source, name] of drifts) {
    assert.throws(
      () => requireExactPropertySignatures(
        interfacePropertySignatures(source, name),
        contracts[name],
        name,
      ),
      new RegExp(`${name}.* changed`),
      kind,
    );
  }
});
